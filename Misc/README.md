# Miscellaneous Resources

A growing collection of resources that don't belong to a specific module but are useful throughout the BSc Computer Science course.

---

## YouTube Channels

| Channel | What to expect |
|---------|----------------|
| [Computerphile](https://www.youtube.com/@Computerphile) | Cryptography, OS, computer architecture — explained by experts |
| [Tom Scott](https://www.youtube.com/@TomScottGo) | Technology, linguistics, and curious concepts — great for sparking curiosity |
| [Linus Tech Tips](https://www.youtube.com/@LinusTechTips) | PC hardware, peripherals, and emerging tech trends |
| [Fireship](https://www.youtube.com/@Fireship) | Keeping up with tech news and new tools espcially with the code report series |

---

## Courses & Tools

- **[Create with Code – Unity Learn](https://learn.unity.com/course/create-with-code)**  
  Free official Unity course. Build games in C# with hands-on projects — a good intro to applied programming beyond the module material.

---

## Newsletters

- **[Tom Scott's Newsletter](https://www.tomscott.com/newsletter/)**  
  Extra context and thoughts behind his videos. Worth subscribing if you already watch his content.
